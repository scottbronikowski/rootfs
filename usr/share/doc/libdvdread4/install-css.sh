...
sitert=http://packages.medibuntu.org/
site=${sitert}pool/free/libd/libdvdcss/
...
if [ "$do_dyn" = "T" ];then
# List source, Karmic: http://www.medibuntu.org/sources.list.d/karmic.list
#	deb http://packages.medibuntu.org/ karmic free non-free
# List http://packages.medibuntu.org/dists/karmic/free/binary-amd64/Packages
    set +e	# prevent this from stopping everything
    wget "${sitert}dists/${dist}/free/binary-${arch}/Packages" -O "$CSSTMP"/Packages && \
	url=${sitert}$(grep "Filename: .*libdvdcss${soname}.*\.deb" "$CSSTMP"/Packages|sed  's/Filename: //'|head -n 1) && \
