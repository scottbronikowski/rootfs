   ...
   (eval-when-compile
	...
	"(" (regexp-opt
	     '("begin"
	       ...
	       "syntax" "syntax-rules"
	       ;; added by Qobi
	       "when" "unless" "set!" "while" "define-structure"
	       "define-structure-external" "define-c-external" "either"
	       "for-effects" "one-value" "local-one-value" "all-values"
	       "possibly?" "necessarily?" "upon-failure" "local-set!" "lazy"
	       "define-toggle-button" "define-radio-buttons"
	       "define-cycle-button" "define-integer-range-buttons"
	       "define-display-pane-application" "define-application"
	       "define-command" "define-primitive-procedure"
	       "primitive-procedure" "foreign-procedure"
	       "define-macro" "include" "parallel-begin" "parallel-call"
	       "parallel-do" "mutex-begin") t)
	"\\>") 1)
      ;;
