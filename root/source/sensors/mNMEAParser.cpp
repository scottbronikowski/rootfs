...
//		return;
	...
	strncpy(auxBuf, p1, 2);
	auxBuf[2] = '\0';
	//printf("day as string = %s\n", auxBuf);
	day = atoi(auxBuf);
	//printf("day as d = %d, as u = %u\n", day, day);
	p1 += 2;
	strncpy(auxBuf, p1, 2);
	auxBuf[2] = '\0';
	month = atoi(auxBuf);
	...
	//printf("after magneticVariation\n");
	if((p2 = strchr(p1, '*')) == NULL)
		return;
	//printf("after first return\n");
	// Set the values of m_GPSInfo
	m_GPSInfo.m_latitude = latitude;
	//printf("m_latitude = %.3f\n", m_GPSInfo.m_latitude);
	m_GPSInfo.m_longitude = longitude;
	m_GPSInfo.m_nSentences++;
	...
	m_GPSInfo.m_day = day;
	//printf("m_day as d = %d, as u = %u\n", m_GPSInfo.m_day, m_GPSInfo.m_day);
	...
	if(p2 - p1 > 1)
		return;
	//printf("after second return\n");
	if(*p1 == 'W')
	{
	  latitude = -latitude;
	  m_GPSInfo.m_latitude = latitude;
	  //printf("m_latitude reassigned = %.3f\n", m_GPSInfo.m_latitude);
	}
	else if(*p1 != 'E' && *p1 != '*')
		return;
	//printf("after third return\n");
