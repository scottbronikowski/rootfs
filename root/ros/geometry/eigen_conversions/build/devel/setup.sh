...
# CURRENT_WORKSPACE="/root/ros/geometry/eigen_conversions/build/devel"
...
SETUP_UTIL=/root/ros/geometry/eigen_conversions/build/devel/_setup_util.py
...
if [ $IS_RECURSIVE -eq 0 ]; then
  export CMAKE_PREFIX_PATH="$("$SETUP_UTIL" --prefix --name CMAKE_PREFIX_PATH --value "/root/ros/geometry/eigen_conversions/build/devel:/root/ros_core_ws/install")$CMAKE_PREFIX_PATH"
fi

# source setup.SHELL from parent workspaces
if [ $IS_RECURSIVE -eq 0 ]; then
  _CATKIN_SETUP_NO_RECURSION=1
  for workspace in $("$SETUP_UTIL" --get-reversed-workspaces --value "/root/ros/geometry/eigen_conversions/build/devel"); do
...
export CPATH="$("$SETUP_UTIL" --prefix --name CPATH --value "/root/ros/geometry/eigen_conversions/build/devel$CPATH_dir")$CPATH"
if [ $IS_DARWIN -eq 0 ]; then
  export LD_LIBRARY_PATH="$("$SETUP_UTIL" --prefix --name LD_LIBRARY_PATH --value "/root/ros/geometry/eigen_conversions/build/devel$LD_LIBRARY_PATH_dir")$LD_LIBRARY_PATH"
else
  export DYLD_LIBRARY_PATH="$("$SETUP_UTIL" --prefix --name DYLD_LIBRARY_PATH --value "/root/ros/geometry/eigen_conversions/build/devel$LD_LIBRARY_PATH_dir")$DYLD_LIBRARY_PATH"
fi
export PATH="$("$SETUP_UTIL" --prefix --name PATH --value "/root/ros/geometry/eigen_conversions/build/devel$PATH_dir")$PATH"
export PKG_CONFIG_PATH="$("$SETUP_UTIL" --prefix --name PKG_CONFIG_PATH --value "/root/ros/geometry/eigen_conversions/build/devel$PKG_CONFIG_PATH_dir")$PKG_CONFIG_PATH"
export PYTHONPATH="$("$SETUP_UTIL" --prefix --name PYTHONPATH --value "/root/ros/geometry/eigen_conversions/build/devel$PYTHONPATH_dir")$PYTHONPATH"

# run all environment hooks of this workspace
FIND=`which find`
SORT=`which sort`
ENV_HOOKS_GENERIC=$($FIND "/root/ros/geometry/eigen_conversions/build/devel/etc/catkin/profile.d" -maxdepth 1 -name "*.sh" 2>/dev/null | $SORT)
if [ "$CATKIN_SHELL" != "sh" ]; then
  ENV_HOOKS_SPECIFIC=$($FIND "/root/ros/geometry/eigen_conversions/build/devel/etc/catkin/profile.d" -maxdepth 1 -name "*.$CATKIN_SHELL" 2>/dev/null | $SORT)
else
  ENV_HOOKS_SPECIFIC=
fi
